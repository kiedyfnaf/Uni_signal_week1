...
export async function onRequestGet({ request, env }) {
  try {
    ...
    return json({ error: error.message || 'Could not load manga list.' }, 500);
  }
}
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!title || !author || !String(data.notes || '').trim()) {
      return json({ error: 'Title, author, and notes are required.' }, 400);
    }
    ...
    const response = authErrorResponse(error);
    if (response) return response;
    return json({ error: error.message || 'Could not add manga.' }, 500);
  }
}
