...
// api/_utils.js
var SESSION_COOKIE = "manga_session";
...
async function hashPassword(password, salt = randomHex(16)) {
  ...
  const bits = await crypto.subtle.deriveBits({ name: "PBKDF2", salt: hexToBytes(salt), iterations: 12e4, hash: "SHA-256" }, key, 256);
...
async function currentUser(request, env2) {
  ...
  return env2.DB.prepare(`SELECT users.id, users.username, users.role
...
async function createSession(userId, env2) {
  ...
  await env2.DB.prepare(`INSERT INTO sessions (id, user_id, token_hash, expires_at) VALUES (?, ?, ?, datetime('now', '+30 days'))`).bind(sessionId, userId, tokenHash).run();
  return { name: SESSION_COOKIE, value: token };
}
__name(createSession, "createSession");
function sessionCookie(cookie, secure = true) {
...
function adminNames(env2) {
  return String(env2.ADMIN_USERNAMES || "").split(",").map((name) => name.trim().toLowerCase()).filter(Boolean);
}
__name(adminNames, "adminNames");
function parseManga(row) {
  return { ...row, tags: JSON.parse(row.tags_json || "[]"), panelImages: JSON.parse(row.panel_images_json || "[]"), ratings: JSON.parse(row.ratings_json || "[]"), viewed: Boolean(row.viewed), tags_json: void 0, panel_images_json: void 0, ratings_json: void 0 };
}
__name(parseManga, "parseManga");

// api/manga/[id]/viewed.js
async function onRequestPost({ request, env: env2, params }) {
  const user = await requireUser(request, env2);
  const { viewed } = await readJson(request);
  if (typeof viewed !== "boolean") return json({ error: "Viewed must be true or false." }, 400);
  const manga = await env2.DB.prepare("SELECT id FROM manga WHERE id = ?").bind(params.id).first();
  if (!manga) return json({ error: "Manga not found." }, 404);
  await env2.DB.prepare(`INSERT INTO manga_views (user_id, manga_id, viewed) VALUES (?, ?, ?)
...
async function onRequestPost2({ request, env: env2 }) {
  try {
    ...
    const user = await env2.DB.prepare("SELECT id, username, role, password_hash FROM users WHERE username = ? COLLATE NOCASE").bind(String(username || "").trim()).first();
    if (!user || !await verifyPassword(String(password || ""), user.password_hash)) return json({ error: "Incorrect username or password." }, 401);
    const cookie = await createSession(user.id, env2);
...
async function onRequestPost3({ request, env: env2 }) {
  ...
  if (user) {
    ...
    if (token) await env2.DB.prepare("DELETE FROM sessions WHERE token_hash = ?").bind(await sha256(token)).run();
  }
...
async function onRequestPost4({ request, env: env2 }) {
  try {
    ...
    const role = adminNames(env2).includes(cleanUsername.toLowerCase()) ? "admin" : "user";
    await env2.DB.prepare("INSERT INTO users (id, username, password_hash, role) VALUES (?, ?, ?, ?)").bind(id, cleanUsername, await hashPassword(password), role).run();
    const cookie = await createSession(id, env2);
    return json({ user: { id, username: cleanUsername, role } }, 201, { "Set-Cookie": sessionCookie(cookie, new URL(request.url).protocol === "https:") });
  } catch (error3) {
    if (String(error3.message).includes("UNIQUE")) return json({ error: "That username is already taken." }, 409);
...
// api/manga/[id].js
async function onRequestGet2({ request, env: env2, params }) {
  const user = await requireUser(request, env2);
  const row = await env2.DB.prepare(`SELECT manga.*, users.username AS creatorName, COALESCE(manga_views.viewed, 0) AS viewed
...
async function onRequestGet3({ request, env: env2 }) {
  const user = await requireUser(request, env2);
  const result = await env2.DB.prepare(`SELECT manga.*, users.username AS creatorName, COALESCE(manga_views.viewed, 0) AS viewed
    FROM manga JOIN users ON users.id = manga.creator_id
    LEFT JOIN manga_views ON manga_views.manga_id = manga.id AND manga_views.user_id = ?
...
async function onRequestPost5({ request, env: env2 }) {
  const user = await requireAdmin(request, env2);
  const data = await readJson(request);
  const title2 = String(data.title || "").trim();
  const author = String(data.author || "").trim();
  if (!title2 || !author || !String(data.notes || "").trim()) return json({ error: "Title, author, and notes are required." }, 400);
  const id = crypto.randomUUID();
  await env2.DB.prepare(`INSERT INTO manga (id, title, author, genre, link_url, link_description, cover, chapter, tags_json, notes, cover_image, panel_images_json, ratings_json, creator_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(
    id,
    title2,
    author,
  ...
  return json({ manga: { id, title: title2, author, creatorName: user.username, viewed: false } }, 201);
}
__name(onRequestPost5, "onRequestPost");

// ../.wrangler/tmp/pages-oAnjls/functionsRoutes-0.5286133178100978.mjs
var routes = [
  {
