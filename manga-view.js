...
const visitorScores = visitorRatings[title] || [];
...
content.innerHTML = `<section class="view-hero"><div>${cover}</div><div><p class="eyebrow">Manga journal entry</p><h1>${escapeHtml(title)}<span>.</span></h1><p class="view-byline">${escapeHtml(manga.author)} · ${escapeHtml(manga.genre)}</p><div class="view-score"><strong>${combinedAverage.toFixed(1)}</strong><span>combined score / 10<br />My score ${personalAverage.toFixed(1)} · Visitors ${visitorAverage === null ? '—' : visitorAverage.toFixed(1)}</span></div>${tags}</div></section><section class="visitor-rating-box"><div><p class="eyebrow">Visitor rating</p><h2>What do you think?</h2><p>Give this manga a score from 0.0 to 10.0.</p></div><form id="visitorRatingForm"><input id="visitorScore" type="number" min="0" max="10" step="0.1" required placeholder="8.5" aria-label="Your score from 0 to 10" /><button class="button button-dark" type="submit">Rate manga</button></form><span id="visitorRatingStatus" role="status"></span></section><section class="view-layout"><article class="view-notes"><p class="eyebrow">My notes</p><div>${escapeHtml(manga.notes).replace(/\n/g, '<br />')}</div></article><aside class="view-rankings"><div class="panel-heading"><div><p class="eyebrow">Manga ranking</p><h2>20 categories</h2></div><span class="rating-total">${personalAverage.toFixed(1)} / 10</span></div>${ratings.map((rating) => `<div class="view-rating" title="${escapeHtml(ratingDescriptions[rating.category] || 'Personal category score.')}"><span>${escapeHtml(rating.category)} <i>?</i></span><strong>${rating.score.toFixed(1)}</strong><div><i style="width:${rating.score * 10}%"></i></div></div>`).join('')}</aside></section>${panels}`;
document.querySelector('#visitorRatingForm').addEventListener('submit', (event) => {
  ...
  savedRatings[title] = [...(savedRatings[title] || []), score];
